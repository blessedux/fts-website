...
export const DraggableContainer = ({
  ...
  return (
    <GridVariantContext.Provider value={variant}>
      <div className={cn(heightClassName, "overflow-hidden")}>
        <motion.div className={cn(heightClassName, "overflow-hidden")}>
          <motion.div
            className={cn(
              "grid h-fit w-fit cursor-grab grid-cols-[repeat(2,1fr)] bg-[var(--lv2-void)] active:cursor-grabbing will-change-transform",
              className,
            )}
            drag
            dragMomentum={true}
